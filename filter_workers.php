<?php
    ...
    if(isset($_POST["search"]))
    {
        $add_AND = false;
        $add_WHERE = false;
        $sql_statement = "SELECT * FROM workers";
        $filter_states = "";
        ...
        foreach($_POST as $key=>$var)
        {
            if($key != "search" && $key != "click" && $key != "cage" && !strpos($key,"option") && !strpos($key,"2"))
            {
                ...
                if(($key === "wid"  &&!empty($_POST["wid"])) || ($key ==="age" && !empty($_POST["age"])))
                {
                    if($add_AND)
                    {
                ...
                else if($key ==="wid1" || $key == "wid2" || $key ==="age1" || $key ==="age2"){
                    if($key ==="wid1" && !empty($_POST["wid1"]) || ($key ==="age1" && !empty($_POST["age1"]))){
